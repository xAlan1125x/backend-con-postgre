...
export class CategoriaRepository {
  obtenerTodas(): Categoria[] {
    ...
    const resultado = db.prepare("SELECT * FROM categorias").all() as Categoria[];
  ...
  buscarPorId(id: number): Categoria | undefined {
    ...
    const resultado = db.prepare("SELECT * FROM categorias WHERE id = ?").get(id) as Categoria | undefined;
    if (!resultado) {
  ...
  guardar(dto: CrearCategoriaDto): Categoria {
  ...
  actualizar(id: number, dto: ActualizarCategoriaDto): Categoria | undefined {
