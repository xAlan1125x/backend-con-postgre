...
export class PedidoService {
    async checkout(dto) {
        ...
        return prisma.$transaction(async (tx) => {
            ...
            const pedido = await tx.pedido.create({ data: { usuarioId: dto.usuarioId, total: 0 } });
            let total = 0;
            for (const item of dto.productosComprados) {
                ...
                await tx.detallePedido.create({
                    data: { pedidoId: pedido.id, productoId: producto.id, cantidad: item.cantidad, precio: producto.precio }
                });
            }
